...
- SSR 사용시에 요청들어오면 데이터 연동 후 html 생성 반환(오래걸릴 수 있음)
- SSG 사용시에 빌드 타임에 데이터 연동 후 html 이미 완성(오래걸리지 않음)
- SSG 는 자주 업데이트 하는 곳에는 사용하지 않도록 하자.

## SSG 작성해 보기

### /src/pages/index.tsx

- getStaticProps 함수 작성
...
- 동적인 경로
- 상품 상세 소개하는 부분은 내용이 자주 바뀌지 않음.
- 라우터가 바뀌는 것을 Next 가 파악함
- http://localhost:3000/good/1, http://localhost:3000/good/2 ...
- `동적인 경로인 경우 SSG 를 적용하려면 getStaticPaths 가 필요함`
- 빌드 타임에 데이터 연동 파일을 미리 생성
- 경로를 미리 알려줘야 함, `getStaticPaths` 가 역할을 함
...
export function getStaticPaths() {
  return {
    ...
    // fallback: false, // 위의 paths 에 없는 경로는 404로 출력
