...
- 모든 페이지에 적용할 레이아웃은 `_app.tsx` 에 적용한다.
...
- /src/components/global-layout.module.css 생성
...
- /src/pages/components/good-item.tsx
...
- /src/pages/components/good-item.tsx 수정
...
- Next 는 스크롤시 화면에 이미지가 보일때 쯤 로딩함.
- lazy loading 을 지원함.
...
export default function Page() {
  ...
  const { title, price, description, category, image, rating } = mockData;
  return (
    <div className={styles.container}>
